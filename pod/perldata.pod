...
There's two package separators in Perl: A double colon (C<::>) and a single
quote (C<'>).  Normal identifiers can start or end with any number
of double colons, and can contain several parts delimited by double colons.
Single quotes have similar rules, but with some exceptions:
they are not legal at the end of an identifier, and you can't use multiple
continuos single quotes;  That is, C<$'foo> and C<$foo'bar> are legal, but 
C<$''foo> and C<$foo''bar> are not.

...
restrictions.
Also note that if you aren't working under the effect of the C<use utf8;>
pragma, the following only applies to the Latin-1 range.

A basic identifier is one that matches the regex

    / (?: (?=\p{Word}) [\p{XID_Start}_] \p{XID_Continue}* )+ /x

That is, the intersection between the Word and XIDS properties
(plus the underscore), followed by any number of XIDC characters,
one or more times.

Moreso, normal identifiers can start or end with any number
...
may optionally be enclosed in braces.  So the regex above becomes
    ...
    /
        (?(DEFINE)
            ...
            (?<basic_identifier>
                (?: (?=\p{Word}) [\p{XID_Start}_] \p{XID_Continue}* )+
            )
            (?<sigil> [$%@&*])
        )
    /x
...
by Perl; And because they have special parsing rules, they can't be
fully-qualified.  These identifiers come in four forms:
...
=item A sigil, followed by bareword text in brackets where the first character
is a caret, like C<${^GLOBAL_PHASE}>.

=item A sigil followed by a single character that doesn't match any of the
previous rules, like C<$!> or C<%+>, where the character following the
sigil must match the \p{POSIX_Punct} property.

...
Perl now restricts the set of characters allowed in length-one variables.
See L<Illegal character %s (%s) in variable name|perldiag/Illegal character %s (%s) in variable name>
and L<Identifier parsing|perldata/Identifier parsing>.
